...
from CommonTest.RASTest.CPX.TestLoader import *
...
def Testcase_EDMA_002():
    ...
    bmc_web.blackbox_switch(True)
    itp.halt()
    _inject_a = f"socket={msocket}, channel={ch}, dimm={dimm}, rank={rank}, bank_group=1, bank=1, errType='ce'"
    excmd(Cmd.inj_mem.format(_inject_a), delay=Delay.mem, _halt=False, _go=False)
    _inject_b = f"socket={msocket}, channel={ch}, dimm={dimm}, rank={rank}, bank_group=2, bank=2, errType='ce'"
    excmd(Cmd.inj_mem.format(_inject_b), delay=Delay.mem, _halt=False, _go=False)
    _inject_c = f"socket={msocket}, channel={ch}, dimm={dimm}, rank={rank}, bank_group=3, bank=2, errType='ce'"
