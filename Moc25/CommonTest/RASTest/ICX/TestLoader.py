...
from CommonTest.RASTest.ICX.TestDiag import BmcStatus
...
def inj_mem(count=1, dev=1, dev0msk=0x2, socket=Loc.msocket, channel=Loc.ch, dimm=Loc.dimm, rank=Loc.rank,
            bank_group=Loc.bg, bank=Loc.ba, errType="ce", addr=None, delay=Delay.mem):
    dev1msk = 0
    if errType == "uce":
    ...
    if addr:
        ...
        return
    for i in range(count):
        argvs = "socket={}, channel={}, dimm={}, rank={}, bank_group={}, bank={}, errType={}".format(
            socket, channel, dimm, rank, bank_group, bank, repr(errType))
        excmd(Cmd.inj_mem.format(argvs), delay=delay)


def inj_mem_rc(count=0, dev0=1, dev0msk=0x2, socket=Loc.msocket, channel=Loc.ch, dimm=Loc.dimm, rank=Loc.rank,
               bank_group=0, bank=0, row=0, column=0, delay=Delay.mem):
...
def assert_reg(reg_dict: dict):
    ...
    for key, value in reg_dict.items():
        if excmd(key) != value:
            logcustom(f"[Assert] {key}={hex(value)} fail", "ired")
            _fail[key] = value
    assert not _fail
...
def ping_sut(timeout=BOOT_TIMEOUT):
    logcustom("## Ping OS ip: {} ...".format(OS_IP), "icyan")
...
def time_sync():
    ...
    if os_ssh.open_shell():
        logcustom("## OS time zone sync: UTC+8", "iyellow")
        os_ssh.exec_cmds(['timedatectl set-timezone "Asia/Shanghai"'])
        logcustom("## OS date time sync: {}".format(now), "iyellow")
...
def bmc_dump(ssh, path, name):
    ...
    if ssh.execute_command(cmd_diag):  # default close session, re-open in next step
        logcustom("## Dump success, copy to folder:", "icyan")
        logcustom(f"## {path}", "icyan")
        ssh.login()
        sftp = ssh.ssh_client.open_sftp()
        ...
        if os.path.exists(dump_file):
            logcustom("## Copy dump log success!", "igreen")
            return dump_file
...
def check_info(cmd, path=None, logname=""):
    ...
    if path:
        test_dir = os.path.join(os.path.join(os.getcwd(), TEST_FLAG), path)
    ...
    def setting_decorator(func):
        ...
        def wrapped_function(*args, **kwargs):
            ...
            if bmc.login():
                ...
                time.sleep(30)
                ping_sut()
                logcustom('## Set BIOS config to "DFX"...', "icyan")
    ...
    def func_decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if not os.path.exists(dump):
                logcustom("## BMC dump_info.tzr.gz not found, skip bmc diag", "ired")
                return func_rtn
            if error is None:
            ...
            if not hasattr(bmc_diag, error):
                logcustom(f"## Error check not defined: \"{error}\"", "ired")
                DIAG_STATUS[func.__name__] = None  # Error
                return func_rtn
            err_type = getattr(bmc_diag, error)
            logcustom(f"## Start to check bmc dump for: {error}", "iyellow")
            try:
...
def start_test():
    ...
    if bmc.login():
        logcustom("## BMC login success.", "igreen")
        bmc.interaction(["maint_debug_cli\n", "attach diag\n", "dump_state 6\n", "bye\n"],
                        ["%", "Success", "Set dump_print_enable is 1", "Byebye"])
    logcustom("## FDM log enabled success.", "igreen")
    ping_sut()
    time_sync()
    for func in feature:
        # 单个Case测试
