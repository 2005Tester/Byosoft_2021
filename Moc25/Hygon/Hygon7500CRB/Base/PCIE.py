...
def pcie_max_payload():
    ...
    assert SetUpLib.enter_menu_change_value(Key.DOWN,SutConfig.PCIE.SET_PAYLOAD_128B,6)
    time.sleep(2)
...
def Above_4G_Decoding():
    ...
    if BDF:
        ...
        logging.info('SAS卡不存在，跳过SAS卡测试')
    assert SetUpLib.boot_to_setup()
    assert SetUpLib.enter_menu_change_value(Key.DOWN,Sut01Config.PCIE.SET_4G_DECODING_ENABLED,6)
    time.sleep(2)
    SetUpLib.send_keys(Key.SAVE_RESET)
    time.sleep(2)
    if count_enable >= 1:
