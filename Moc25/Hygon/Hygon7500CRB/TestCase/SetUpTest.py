...
from Hygon7500CRB.Base import SetUp
...
def sriov():
    ...
    try:
        ...
        logging.error(e)
        return core.Status.Fail
# @core.test_case(('603','[TC603]Wake Online','网络唤醒'))
# def wake_online():
#
#     try:
...
def SATA_or_PCIE_Switch():
    ...
    try:
        ...
        SetUpLib.boot_to_setup()
        SetUpLib.boot_to_page(SutConfig.Msg.PAGE_ADVANCED)
        SetUpLib.locate_option(Key.DOWN, ['SATA\/PCIE Switch'], 20)
        time.sleep(1)
        SetUpLib.send_key(Key.ENTER)
        time.sleep(1)
        SetUpLib.select_option_value(Key.DOWN, ['J112'], Key.DOWN, 'SATA', 4)
        time.sleep(2)
        SetUpLib.send_keys(Key.SAVE_RESET)
        logging.error(e)
        return core.Status.Fail
