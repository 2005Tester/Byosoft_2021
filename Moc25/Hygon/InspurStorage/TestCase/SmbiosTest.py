...
from InspurStorage.Base import Smbios
import re
...
def smbios():
    ...
    try:
        ...
        with open('InspurStorage/Tools/SMBIOS/type0.txt', 'r') as f:
            content_log1 = f.read()
        ...
        logging.error(e)
        result.log_fail(capture=True)
    types=[1,2,3,4,7,9,13,16,17,20,127]
    count=1
    for i in types:
        tc=('{0}'.format(str(count+102)),'[TC{0}]SMBIOS Type {1}'.format(str(count+102),i),'检查 SMBIOS Type {0}'.format(i))
        result = ReportGen.LogHeaderResult(tc)
        try:
            differs=Smbios.check_diff('{0}type{1}.txt'.format(SutConfig.Env.SMBIOS,str(i)),'InspurStorage/Tools/SMBIOS/type{0}.txt'.format(str(i)))
            if len(differs)==0:
                result.log_pass()
            else:
...
def type13():
    tc=('121', '[TC121]type13', 'type13')
    result=ReportGen.LogHeaderResult(tc)
    try:
        assert Smbios.type13()
        result.log_pass()
        return True
    except Exception as e:
        logging.error(e)
...
def update_smbios():
    tc=('122', '[TC122]Update Smbios', '更新SBMBIOS信息')
    result=ReportGen.LogHeaderResult(tc)
    try:
        assert Smbios.update_smbios()
        result.log_pass()
        return True
    except Exception as e:
        logging.error(e)
