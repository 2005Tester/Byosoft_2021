...
class Pxe:
    ...
    # UEFI模式，PXE一个循环中开始的标志(一般为PXE第一个启动项名称)
    PXE_START_MSG = 'UEFI Intel\(R\) I350 Gigabit Network Connection PXE IPv4'
    # UEFI模式，PXE一个循环中结束的标志(一般为PXE最后一个启动项名称)
    PXE_END_MSG = 'UEFI Intel\(R\) I350 Gigabit Network Connection-2 PXE IPv4'
    # PXE第一个启动项名称
    PXE_BOOT_NAME1 = 'UEFI Intel\(R\) I350 Gigabit Network Connection PXE IPv4'
    # PXE最后一个启动项名称
    PXE_BOOT_NAME2 = 'UEFI Intel\(R\) I350 Gigabit Network Connection-2 PXE IPv4'

    LOC_PXE = ['User Wait Time', 'Network Adapter']
    LOC_USB = ['User Wait Time',
               'USB Flash Drive/USB Hard Disk|USB Flash Drive']
    LOC_BOOT_MANAGER = ['Load Setup Defaults', 'Boot Manager']
    BOOT_MANAGER_MSG = 'Boot Manager Menu'
    SET_UEFI = ['User Wait Time', {'Boot option filter': 'UEFI Only'}, {
        'Network Boot': 'Enabled'}, {'Net Boot IP Version': 'IPv4'}, {'Http Boot': 'Disabled'}]
    SET_PXE_0 = ['User Wait Time', {'PXE Retry': 0}]
    SET_PXE_N = ['User Wait Time', {'PXE Retry': 8}]
    SET_PXE_MAX = ['User Wait Time', {'PXE Retry': 255}]

    # Legacy模式，PXE一个循环中开始的标志
    LEGACY_PXE_START_MSG = '\x0cIntel\(R\)BootAgentGEv1.5.88'
    ...
    SET_LEGACY = ['User Wait Time', {
        'Boot option filter': 'Legacy Only'}, {'Network Boot': 'Enabled'}]
...
class Tool:
    ...
    OPTION_PATH = ['PCIE Configuration', 'PCIE Configuration', 'ESC', 'Video Configuration', 'ESC',
                   ...
                   'ESC', 'USB Configuration', 'ESC', 'Above 4G Decoding', 'Server Configuration', 'ESC',
                   'Wake from RTC']
    ...
    # 改变的SetUp选项
    CHANGE_OPTIONS = ['PCIE Configuration', 'PCIE Configuration', {'PCIE Wake': 'Disabled'},
