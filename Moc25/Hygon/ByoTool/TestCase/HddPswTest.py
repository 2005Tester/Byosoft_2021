...
def get_hdd_msg(type='tcg', types=None):
    if types is None:
        hdd_name = []
        hdd_boot_name = []
        if re.search(type, SutConfig.Psw.HDD_NAME1, re.I):
            hdd_name.append(SutConfig.Psw.HDD_NAME1)
            hdd_boot_name.append(SutConfig.Psw.HDD_BOOT_NAME1)
        if re.search(type, SutConfig.Psw.HDD_NAME2, re.I):
            hdd_name.append(SutConfig.Psw.HDD_NAME2)
            hdd_boot_name.append(SutConfig.Psw.HDD_BOOT_NAME2)
        if hdd_name:
        ...
        if types == ['tcg', 'ata'] or types == ['ata', 'tcg']:
            if re.search('tcg', SutConfig.Psw.HDD_NAME1, re.I) and re.search('ata', SutConfig.Psw.HDD_NAME2, re.I):
                return SutConfig.Psw.HDD_NAME1, SutConfig.Psw.HDD_NAME2, SutConfig.Psw.HDD_BOOT_NAME1, SutConfig.Psw.HDD_BOOT_NAME2
            elif re.search('ata', SutConfig.Psw.HDD_NAME1, re.I) and re.search('tcg', SutConfig.Psw.HDD_NAME2, re.I):
                return SutConfig.Psw.HDD_NAME1, SutConfig.Psw.HDD_NAME2, SutConfig.Psw.HDD_BOOT_NAME1, SutConfig.Psw.HDD_BOOT_NAME2
            else:
                return None
        elif types == ['tcg', 'tcg']:
            if re.search('tcg', SutConfig.Psw.HDD_NAME1, re.I) and re.search('tcg', SutConfig.Psw.HDD_NAME2, re.I):
                return SutConfig.Psw.HDD_NAME1, SutConfig.Psw.HDD_NAME2, SutConfig.Psw.HDD_BOOT_NAME1, SutConfig.Psw.HDD_BOOT_NAME2
            else:
                return None
        elif types == ['ata', 'ata']:
            if re.search('ata', SutConfig.Psw.HDD_NAME1, re.I) and re.search('ata', SutConfig.Psw.HDD_NAME2, re.I):
                return SutConfig.Psw.HDD_NAME1, SutConfig.Psw.HDD_NAME2, SutConfig.Psw.HDD_BOOT_NAME1, SutConfig.Psw.HDD_BOOT_NAME2
            else:
                return None
        else:
...
def boot_to_setup(key=SutConfig.Msg.SETUP_KEY, is_del=True, password='admin'):
    if password == 'admin':
    ...
    while try_counts:
        ...
        if result == [True, True]:
            if is_del:
                SetUpLib.enter_menu_change_value(Key.DOWN, SutConfig.Psw.LOC_HDD_PSW + [SutConfig.Psw.HDD_NAME1], 18)
                ...
                SetUpLib.enter_menu_change_value(Key.DOWN, SutConfig.Psw.LOC_HDD_PSW + [SutConfig.Psw.HDD_NAME2], 18)
                if SetUpLib.wait_message(SutConfig.Psw.DISABLE_PSW_NAME, 3):
                    SetUpLib.locate_menu(Key.DOWN, SutConfig.Psw.DISABLE_PSW_NAME, 10)
                    PwdLib.del_hdd_psw(password, True)
...
def boot_to_setup_two(key=SutConfig.Msg.SETUP_KEY, is_del=True):
    if PwdLib.HDD_PW.ADMIN and PwdLib.HDD_PW.ADMIN_ANOTHER:
        ...
        if is_del:
            assert SetUpLib.locate_menu(Key.DOWN, SutConfig.Psw.LOC_HDD_PSW + [SutConfig.Psw.HDD_NAME1,
                                                                               SutConfig.Psw.DISABLE_PSW_NAME], 18)
            assert PwdLib.del_hdd_psw(PwdLib.HDD_PW.ADMIN, True)
            assert SetUpLib.locate_menu(Key.DOWN, SutConfig.Psw.LOC_HDD_PSW + [SutConfig.Psw.HDD_NAME2,
                                                                               SutConfig.Psw.DISABLE_PSW_NAME], 18)
...
@core.test_case(('5000', '[TC5000]HDD Password Security 000', '硬盘密码前提测试'))
def hdd_password_5000():
    """
    Name:   硬盘密码前提测试
    ...
    try:
        ...
        if not re.search(SutConfig.Psw.HDD_NAME1, data):
            SutConfig.Psw.HDD_NAME1 = ''
            logging.info('硬盘1名错误')
            count += 1
        else:
            assert SetUpLib.locate_menu(Key.DOWN, [SutConfig.Psw.HDD_NAME1,
        ...
        if not re.search(SutConfig.Psw.HDD_NAME2, data):
            SutConfig.Psw.HDD_NAME2 = ''
            logging.info('硬盘2名错误')
            count += 1
        else:
            assert SetUpLib.enter_menu_change_value(Key.DOWN, SutConfig.Psw.LOC_HDD_PSW, 18)
            assert SetUpLib.locate_menu(Key.DOWN, [SutConfig.Psw.HDD_NAME2, SutConfig.Psw.ENABLE_HDD_NAME],
                                        5)
            if not re.search(f'{str(PwdLib.HDD_PW.MIN)}\s*to\s*{str(PwdLib.HDD_PW.MAX)}', SetUpLib.help_msg):
                min = re.findall('(\d+)\s*to\s*\d+', SetUpLib.help_msg)[0] if re.findall('(\d+)\s*to\s*\d+',
                                                                                         SetUpLib.help_msg) else None
...
@core.test_case(('5001', '[TC5001]HDD Password Security 001', 'TCG协议硬盘密码设置检查'))
def hdd_password_5001():
    """
    Name:   TCG协议硬盘密码设置检查
...
@core.test_case(('5002', '[TC5002]HDD Password Security 002', 'TCG协议设置硬盘密码的哈希算法'))
def hdd_password_5002():
    """
...
@core.test_case(('5003', '[TC5003]HDD Password Security 003', 'TCG协议硬盘管理员密码长度边界值校验'))
def hdd_password_5003():
    """
...
@core.test_case(('5004', '[TC5004]HDD Password Security 004', 'TCG协议硬盘管理员密码复杂度检查'))
def hdd_password_5004():
    """
...
@core.test_case(('5005', '[TC5005]HDD Password Security 005', 'TCG协议更改硬盘管理员密码'))
def hdd_password_5005():
    """
    Name:   TCG协议更改硬盘管理员密码
...
@core.test_case(('5006', '[TC5006]HDD Password Security 006', 'TCG协议删除硬盘管理员密码'))
def hdd_password_5006():
    """
    Name:   TCG协议删除硬盘管理员密码
...
@core.test_case(('5007', '[TC5007]HDD Password Security 007', 'TCG协议硬盘用户密码长度边界值校验'))
def hdd_password_5007():
    """
...
@core.test_case(('5008', '[TC5008]HDD Password Security 008', 'TCG协议硬盘用户密码复杂度检查'))
def hdd_password_5008():
    """
...
@core.test_case(('5009', '[TC5009]HDD Password Security 009', 'TCG协议修改硬盘用户密码'))
def hdd_password_5009():
    """
    Name:   TCG协议修改硬盘用户密码
...
@core.test_case(('5010', '[TC5010]HDD Password Security 010', 'TCG协议管理员,用户密码测试'))
def hdd_password_5010():
    """
...
@core.test_case(('5011', '[TC5011]HDD Password Security 011', 'TCG协议删除硬盘管理员&用户密码'))
def hdd_password_5011():
    """
...
@core.test_case(('5012', '[TC5012]HDD Password Security 012', 'TCG协议硬盘密码权限检查'))
def hdd_password_5012():
    """
    Name:   TCG协议硬盘密码状态检查
    ...
    try:
        ...
        assert SetUpLib.boot_with_hotkey_only(SutConfig.Msg.SETUP_KEY, SutConfig.Msg.SETUP_MESSAGE, 200, SutConfig.Msg.POST_MESSAGE)
        assert SetUpLib.enter_menu_change_value(Key.DOWN, SutConfig.Psw.LOC_HDD_PSW + [hdd_name], 18)
        assert SetUpLib.verify_info(
            [SutConfig.Psw.HDD_PSW_STATUS_INSTALLED, SutConfig.Psw.ERASE_NAME, SutConfig.Psw.DISABLE_PSW_NAME,
             SutConfig.Psw.CHANGE_ADMIN_NAME, SutConfig.Psw.CHANGE_USER_NAME], 6)
...
@core.test_case(('5013', '[TC5013]HDD Password Security 013', 'TCG协议硬盘密码输错测试'))
def hdd_password_5013():
    """
    Name:   TCG协议硬盘密码输错测试
...
@core.test_case(('5014', '[TC5014]HDD Password Security 014', 'TCG协议硬盘密码ESC跳过测试'))
def hdd_password_5014():
    """
...
@core.test_case(('5015', '[TC5015]HDD Password Security 015', 'TCG协议硬盘密码校验测试'))
def hdd_password_5015():
    """
    Name:   TCG协议硬盘密码校验测试
...
@core.test_case(('5016', '[TC5016]HDD Password Security 016', 'TCG协议硬盘密码更新BIOS'))
def hdd_password_5016():
    """
...
@core.test_case(('5031', '[TC5031]HDD Password Security 031', 'ATA协议硬盘密码检查设置'))
def hdd_password_5031():
    """
    Name:   ATA协议硬盘密码检查设置
...
@core.test_case(('5032', '[TC5032]HDD Password Security 032', '设置ATA协议硬盘密码的哈希算法'))
def hdd_password_5032():
    """
...
@core.test_case(('5033', '[TC5033]HDD Password Security 033', 'ATA协议硬盘用户密码长度边界值校验'))
def hdd_password_5033():
    """
...
@core.test_case(('5034', '[TC5034]HDD Password Security 034', 'ATA协议硬盘管理员密码长度边界值校验'))
def hdd_password_5034():
    """
...
@core.test_case(('5035', '[TC5035]HDD Password Security 035', 'ATA协议硬盘用户密码复杂度检查'))
def hdd_password_5035():
    """
...
@core.test_case(('5036', '[TC5036]HDD Password Security 036', 'ATA协议硬盘管理员密码复杂度检查'))
def hdd_password_5036():
    """
...
@core.test_case(('5037', '[TC5037]HDD Password Security 037', '更改ATA协议硬盘用户密码'))
def hdd_password_5037():
    """
    Name:   更改ATA协议硬盘用户密码
...
@core.test_case(('5038', '[TC5038]HDD Password Security 038', '修改ATA协议硬盘管理员密码'))
def hdd_password_5038():
    """
    Name:   修改ATA协议硬盘管理员密码
    ...
    Steps:  1.设置硬盘用户密码
            ...
            5.选择“ 更改硬盘管理员密码”，输入正确的硬盘用户密码后，输入符合硬盘密码规则的新的硬盘管理员密码。
            6.选择“ 更改硬盘管理员密码”，输入正确的管理员密码后，输入不符合硬盘密码规则的新的硬盘管理员密码。
            7.删除硬盘密码
...
@core.test_case(('5039', '[TC5039]HDD Password Security 039', '删除ATA协议硬盘密码'))
def hdd_password_5039():
    """
    Name:   删除ATA协议硬盘密码
...
@core.test_case(('5040', '[TC5040]HDD Password Security 040', 'ATA协议管理员,用户密码测试'))
def hdd_password_5040():
    """
    ...
    try:
        ...
        # assert PwdLib.check_psw_post(new_admin, '')
        # data = SetUpLib.boot_to_boot_menu(True, False)
        # assert re.search(hdd_boot_name, data)
        # assert SetUpLib.select_boot_option(Key.UP, SutConfig.Msg.ENTER_SETUP, 30, SutConfig.Msg.SETUP_MESSAGE)
        # assert SetUpLib.enter_menu_change_value(Key.DOWN, SutConfig.Psw.LOC_HDD_PSW + [hdd_name], 18)
...
@core.test_case(('5041', '[TC5041]HDD Password Security 041', 'ATA协议NVME硬盘密码状态检查'))
def hdd_password_5041():
    """
    Name:   TCG协议硬盘密码状态检查
...
@core.test_case(('5042', '[TC5042]HDD Password Security 042', 'ATA协议SATA硬盘密码状态检查'))
def hdd_password_5042():
    """
    Name:   TCG协议硬盘密码状态检查
...
@core.test_case(('5043', '[TC5043]HDD Password Security 043', 'ATA协议硬盘密码输错测试'))
def hdd_password_5043():
    """
    Name:   ATA协议硬盘密码输错测试
...
@core.test_case(('5044', '[TC5044]HDD Password Security 044', 'ATA协议硬盘密码ESC跳过测试'))
def hdd_password_5044():
    """
...
@core.test_case(('5045', '[TC5045]HDD Password Security 045', 'ATA协议NVME硬盘密码校验测试'))
def hdd_password_5045():
    """
...
@core.test_case(('5046', '[TC5046]HDD Password Security 046', 'ATA协议SATA硬盘密码校验测试'))
def hdd_password_5046():
    """
...
@core.test_case(('5047', '[TC5047]HDD Password Security 047', 'ATA协议硬盘密码更新BIOS'))
def hdd_password_5047():
    """
...
@core.test_case(('5061', '[TC5061]HDD Password Security 061', '多硬盘密码输错测试(两个NVME)'))
def hdd_password_5061():
    """
...
@core.test_case(('5062', '[TC5062]HDD Password Security 062', '多硬盘ESC跳过密码测试(两个NVME)'))
def hdd_password_5062():
    """
...
@core.test_case(('5063', '[TC5063]HDD Password Security 063', '多硬盘密码输错测试(一个NVME，一个SATA)'))
def hdd_password_5063():
    """
...
@core.test_case(('5064', '[TC5064]HDD Password Security 064', '多硬盘ESC跳过密码测试(一个NVME，一个SATA)'))
def hdd_password_5064():
    """
...
@core.test_case(('5065', '[TC5065]HDD Password Security 065', '多硬盘密码输错测试(两个SATA)'))
def hdd_password_5065():
    """
...
@core.test_case(('5066', '[TC5066]HDD Password Security 066', '多硬盘ESC跳过密码测试(两个SATA)'))
def hdd_password_5066():
    """
