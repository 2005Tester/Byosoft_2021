...
def cpu_information():
    ...
    cpu_core = re.findall(r'CPU Core Count *?(\d{1,2}) Cores', data)
    cpu_thread = re.findall(r'CPU Core Count *?\d{1,2} Cores (\d{1,2}) Threads', data)
    ...
    while True:
        ...
        if thread_result:
            break
    linux_cpu_thread=re.findall(r'Thread Count: (\d{1,2})',thread_result[0])
...
def cpu_hyper_threading():
    ...
    SetUpLib.send_key(Key.ENTER)
    time.sleep(1)
    if SetUpLib.wait_message('Cores {0} Threads'.format(thread_count_close),10):
    ...
    assert SetUpLib.enter_menu_change_value(Key.DOWN, SutConfig.Msg.CPU_INFO, 7)
    SetUpLib.send_key(Key.ENTER)
    if SetUpLib.wait_message('Cores {0} Threads'.format(thread_count_open),10):
...
def cpu_cstate():
    ...
    assert SetUpLib.boot_to_setup()
    assert SetUpLib.enter_menu_change_value(Key.DOWN,Sut01Config.Cpu.SET_CSTATE_ENABLED,20)
    time.sleep(2)
    SetUpLib.send_keys(Key.SAVE_RESET)
    time.sleep(5)
    assert SetUpLib.boot_os_from_bm()
