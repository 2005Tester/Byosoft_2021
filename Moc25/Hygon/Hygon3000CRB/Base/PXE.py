...
def pxe_option_rom():
    ...
    if 'PXE' not in datas:
        ...
        return
    assert SetUpLib.select_boot_option(Key.DOWN,Sut01Config.Msg.ENTER_SETUP,20,SutConfig.Msg.PAGE_MAIN)
    assert SetUpLib.enter_menu_change_value(Key.DOWN,Sut01Config.PXE.SET_NETWORK_BOOT_ENABLED,15)
    time.sleep(1)
    SetUpLib.send_keys(Key.SAVE_RESET)
    if not SetUpLib.wait_message(SutConfig.Msg.POST_MESSAGE):
        BmcLib.init_sut()
...
def pxe_boot():
    ...
    SetUpLib.send_keys(Key.SAVE_RESET)
    time.sleep(2)
    assert SetUpLib.boot_with_hotkey(Key.F12, Sut01Config.Msg.PXE_IPV4_MSG, 150, Sut01Config.Msg.POST_MESSAGE)
    logging.info('IPv4PXE启动成功')
    assert SetUpLib.boot_to_setup()
    assert SetUpLib.enter_menu_change_value(Key.DOWN, Sut01Config.PXE.SET_IP_VERSION_IPv6,6)
    time.sleep(2)
    SetUpLib.send_keys(Key.SAVE_RESET)
    time.sleep(2)
...
def pxe_retry():
    ...
    time.sleep(20)
    if BmcLib.ping_sut():
        logging.info('网络启动重试设置为0次，启动失败后，不再重新尝试，直接进入系统')
    else:
        return
    assert SetUpLib.boot_to_setup()
    ...
    counts=0
    while True:
        if SetUpLib.wait_message_enter(Sut01Config.Msg.PXE_IPV4_MSG_ERROR):
            counts+=1
        ...
        if counts==try_counts:
            ...
            #     return
            # break
    logging.info('网络启动重试设置为3次，启动失败3次后，不再尝试启动。')
    return True
...
def Http_Boot():
    assert SetUpLib.boot_to_setup()
    assert SetUpLib.enter_menu_change_value(Key.DOWN, Sut01Config.PXE.SET_HTTP_ENABLED1, 10)
    time.sleep(2)
    SetUpLib.send_keys(Key.SAVE_RESET)
    time.sleep(5)
    assert SetUpLib.boot_with_hotkey(Key.F7, Sut01Config.Msg.ENTER_BOOTMENU_CN, 150, Sut01Config.Msg.POST_MESSAGE)
    assert SetUpLib.locate_option(Key.DOWN,[Sut01Config.Msg.HTTP_NAME1],8)
    time.sleep(1)
    SetUpLib.send_key(Key.ENTER)
    data = SetUpLib.get_data(10)
    if Sut01Config.Msg.HTTP_IPV4_MSG in data:
    ...
    time.sleep(5)
    assert SetUpLib.boot_with_hotkey(Key.F7, Sut01Config.Msg.ENTER_BOOTMENU_CN, 150, Sut01Config.Msg.POST_MESSAGE)
    assert SetUpLib.locate_option(Key.DOWN, [SutConfig.Msg.HTTP_NAME2], 8)
    time.sleep(1)
    SetUpLib.send_key(Key.ENTER)
