...
def _check_mem_err_count(socket=Loc.msocket, mc=Loc.imc, ch=Loc.channel, count=1):
    assert excmd(f"sv.socket{socket}.uncore.memss.mc{mc}.ch{ch}.imc{mc}_mc_status_shadow.cor_err_cnt"
...
@check_dump("fdm_memce_array", False,
            [f"{Loc.msocket} {Loc.ch} {Loc.dimm} {Loc.rank} {Loc.bg} {Loc.ba} 1", 3],
            [f"{Loc.msocket} {Loc.ch} {Loc.dimm} {Loc.rank} {Loc.bg} {Loc.ba} 2", 3])
@check_dump("maint_mem_ce",
            [f"{Loc.msocket} {Loc.ch} {Loc.dimm} {Loc.rank} {Loc.bg} {Loc.ba} 1", 3, "DDDC sparing"],
            [f"{Loc.msocket} {Loc.ch} {Loc.dimm} {Loc.rank} {Loc.bg} {Loc.ba} 2", 3, "Memory_CE_Bucket"])
def Testcase_MemRas_ADDDC_018():
    """ADDDC+1功能测试"""
    _trigger_vls(level="bank")
    inj_mem(count=3, socket=Loc.msocket, channel=Loc.ch, dimm=Loc.dimm, dev=2)
    timer(Delay.rank_vls)
    excmd(Cmd.adddc_check.format(Loc.msocket, Loc.imc))
...
def Testcase_MemRas_DDR5_CAP_001():
    """DDR5 Command/Address Parity Check and Retry"""
...
def Testcase_MemRas_Scrubbing_001():
    """Patrol Scrubbing"""
    shell_cmd("dmesg -C")
    cmd = f'socket={Loc.msocket}, channel={Loc.ch}, sub_ch={Loc.sub_ch}, dimm={Loc.dimm}, rank=0, errType="ce", PatrolConsume=True'
    excmd(Cmd.ei_dev.format(1, 2, 3, 0))
...
def Testcase_MemRas_Scrubbing_002():
    """Demand Scrub"""
    excmd(Cmd.dimminfo)
    assert_reg({f"sv.socket{Loc.msocket}.uncore.memss.m2mem{Loc.imc}.defeatures0.demandscrubwrdis":0})
...
def Testcase_MemRas_Scrubbing_003():
    ...
    assert_reg({f"sv.socket{dram_addr.socket}.uncore.memss.m2mem{dram_addr.imc}.defeatures0.demandscrubwrdis":0})
    shell_cmd("dmesg -C")
    excmd(Cmd.dump_mem_err)
...
@bios_setting("PatrolScrub")
@check_dump("fdm_mem_ce", sysaddr=0x20000, count=1, mscode="0X0008", mcacode="0X00C0")
