...
from batf import SerialLib, MiscLib, core, SshLib
...
def check_info():
    ...
    check_list = [Msg.RC_VERSION, FW_VER.BIOS, Msg.BIOS_DATE, FW_VER.BMC, SutConfig.Env.BMC_IP, Msg.CPU_TYPE,
        ...
        for s_str in SutConfig.Env.LogTime_Dedicated + Msg.LogTime_common:
            ...
            if not s_str in str(res):
                if not re.search(r"\d+", s_str):
                    ...
                    return False
        if s_cont == 0:
            logging.info('All elements check')
            return True
...
# OnStart: NA
# OnComplete: NA
@core.test_case(('803', '[TC803] 串口日志打印测试01_03', '【UEFI模式】支持BIOS启动开始和结束信息打印及上报'))
...
# OnStart: fulldebug
# OnComplete: NA
@core.test_case(('804', '[TC804] 串口日志打印测试02', '【全打印开启】支持BIOS启动开始和结束信息打印及上报'))
...
# OnStart: 'UEFI'模式
# Steps:
# '1、系统启动进入Boot Manage界面，查看Post看门狗超时后触发动作，有结果A；
#  2、进入Setup界面，查看Post看门狗超时后触发动作，有结果A；
#  3、进入Device Manage界面，查看Post看门狗超时后触发动作，有结果A；
#  4、进入Secure Boot界面，查看Post看门狗超时后触发动作，有结果A。
#  A：超时未复位。
# OnCompleted: Power Off
def testcase_watchdog_007():
    tc = ('805', '[TC805] 07 人机界面不触发Post看门狗测试', '支持看门狗设置')
    result = ReportGen.LogHeaderResult(tc)
    try:
        assert SetUpLib.boot_to_page(Msg.PAGE_BMC)
        assert SetUpLib.enter_menu(Key.DOWN, ['iBMC WDT Configuration'], 20, 'iBMC')
        assert SetUpLib.set_option_value('iBMC WDT Action For POST', 'PowerCycle')
        ...
        time.sleep(305)
        assert SetUpLib.enter_menu(Key.DOWN, Msg.BOOT_OPTION_SUSE, 8, Msg.SUSE_GRUB)
        assert SetUpLib.boot_to_setup()
        logging.info('Wait for 305s to verify watchdog time out function -  2nd time')
        time.sleep(305)
        SetUpLib.send_keys([Key.RIGHT * 2, Key.ENTER])
        assert SetUpLib.locate_option(Key.DOWN, ["iSCSI Configuration"], 5)
        logging.info('Wait for 305s to verify watchdog time out function -  3rd time')
        time.sleep(305)
        SetUpLib.send_keys([Key.ESC, Key.DOWN, Key.LEFT, Key.ENTER])
        assert SetUpLib.locate_option(Key.DOWN, ["Secure Boot Mode", '<Standard Mode>'], 5)
        logging.info('Wait for 305s to verify watchdog time out function -  4th time')
        time.sleep(305)
        SetUpLib.send_keys([Key.ESC, Key.RIGHT, Key.ENTER])
        assert SerialLib.is_msg_present(Sut.BIOS_COM, Msg.PAGE_INFO)
        result.log_pass()
        return True
    except AssertionError:
        result.log_fail()
...
def testcase_pxeRetry_001():
    ...
    try:
        ...
        # TCE default is Cold Boot
        assert SetUpLib.locate_option(Key.DOWN, ['PXE Retry Count', '\[1\]'], 7) is None
        assert SetUpLib.set_option_value('Boot Fail Policy', 'Boot Retry')
