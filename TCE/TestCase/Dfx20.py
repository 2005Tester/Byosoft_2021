...
def cnd_test_step(value):
    ...
    assert SutConfig.Env.OS_IP in res1 \
           and dev_list.sort() == SutConfig.SysCfg.device_order.sort(), 'check network device order failed'
...
def cnd_support_test01():
    ...
    try:
        assert cnd_test_step(Msg.DISABLED)
        result.log_pass()
...
def cnd_support_test02():
    ...
    result = ReportGen.LogHeaderResult(tc)
    try:
        assert cnd_test_step(Msg.ENABLED)
        result.log_pass()
    except AssertionError:
        result.log_fail(capture=True)
    finally:
...
def testcase_ACPI_002():
    try:
        ...
        assert '2020' or '2022' in res, '{}'.format(res)   # last acpi ver is 6.4,
        return core.Status.Pass
    except Exception as e:
        logging.error(e)
        core.capture_screen()
