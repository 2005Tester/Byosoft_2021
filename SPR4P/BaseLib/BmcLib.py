...
def read_bt_data(key_str, timeout=5):
    """
    key_str: the test_data u wanna grab from bt channel,
    """
    logging.info("[BmcLib] Read bmc bt test_data...")
    ...
    try:
        if Sut.BMC_SSH.login():
            ...
            for i in range(0, len(cmds)):
                ...
                time.sleep(0.2)
                if cmds[i] == trace_cmd:
                    logging.info('BT test_data receiving and searching...')
                    ...
                    while True:
                        ...
                        if (now - start_time) > timeout:
                            logging.info('Grab bt test_data timeout, close the session')
                            op.close()
                            raise Exception
                        time.sleep(0.1)
                else:
                    pass  # skip invalid test_data,
            op.close()
        return True, res
    except Exception as e:
        logging.error(e)
