...
class BmcWeb:
    ...
    def bmc_post(self, url: str, data: dict):
        if self.web_login():
            _post_result = self.session.post(url=url, json=data, headers=self.headers).status_code
            self.web_logout()
            return _post_result == 200

    def bmc_patch(self, url: str, data: dict):
        if self.web_login():
            _patch_result = self.session.patch(url=url, json=data, headers=self.headers).status_code
            self.web_logout()
            return _patch_result == 200

    def bmc_get(self, url: str):
        if self.web_login():
            try:
                _get_result = self.session.get(url=url, headers=self.headers).json()
            except Exception:
                _get_result = {}
            self.web_logout()
            return _get_result

    def blackbox_switch(self, enable=True):  # 黑匣子开关
        _blackbox_url = f"{self.host}/UI/Rest/Maintenance/SystemDiagnostic"
        _status, _message = (True, "Enable") if enable else (False, "Disable")
        print(f"Set Blackbox to \"{_message}\"")
        return self.bmc_patch(url=_blackbox_url, data={"BlackBoxEnabled": _status, "PCIeInterfaceEnabled": _status})
