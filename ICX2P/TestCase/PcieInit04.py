...
# OnStart: NA
# OnComplete: NA
@core.test_case(('638', '[TC638] Testcase_PCIeResource_021', '【Legacy模式】PCIe设备资源一致性测试'))
def pcie_resource_lspci_legacy():
    ...
    def get_lspci_info():
        try:
            ...
            for bdf in bdf_list:
                pcie_cmd = f"lspci -s {bdf} -vvv"
                pci_info = SshLib.execute_command(Sut.OS_LEGACY_SSH, pcie_cmd)
                if not pci_info:
                    return
                lspci_info.append(pci_info)
            return lspci_info
    ...
    def iio_aspm_check(value):  # 修改Global为PerPort，遍历修改IIO并检查状态与OS pci info
        ...
        try:
            ...
            for os_port in os_ports_bdf:
                ...
                assert re.search(f"ASPM.*{aspm_lnkcap_flag.get(save_value)}", port_rtn), f"{os_port} ASPM status fail:\n{port_rtn}"
                logging.info(f"Verify pass: root port {os_port} ASPM = {save_value}")
            return True
        except Exception as e0:
            logging.error(e0)
