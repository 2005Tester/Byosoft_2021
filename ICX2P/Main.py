...
def TestScope():
    ...
    biosTest.dram_rapl_option_check(ser)
    biosTest.securityBoot(ser)
    biosTest.vtd(ser)
    biosTest.cnd_default_enable()
    Cpu.upi_link_status(ser)
    Cpu.cpu_cores_active_enable_1(ser, ssh_os)
    Cpu.cpu_cores_active_enable_middle(ser, ssh_os)
    Cpu.cpu_cores_active_enable_max(ser, ssh_os)
