...
from Hygon7500CRB.BaseLib import BmcLib
...
'''
# Enter setup menu
def enter_menu(key, option_path, try_counts, confirm_msg, timeout=15):
    logging.info("Go to setup menu:{0}".format(option_path))
    for option in option_path:

        logging.info("Locate menu: {0}".format(option))
        if not Sut.BIOS_COM.locate_menu(key, option, try_counts):
            if not Sut.BIOS_COM.locate_menu(Key.UP, option, try_counts): #修复当要找的分区在最上面，导致找不到的问题
...
def locate_option( key, setupoption, try_counts, delay=1.5,readline=False):
    ES = "(\x1B[@-_][0-?]*[ -/]*[@-~]){1,7}"
...
def boot_to_setup():
    ...
    while try_counts:
        ...
        if not boot_with_hotkey_only(Key.F2, SutConfig.Msg.SEL_LANG, 200, SutConfig.Msg.POST_MESSAGE):
...
def boot_with_hotkey_only(key, msg, timeout=150, hotkey_prompt="F2 to enter SETUP", pw_prompt="fdhgdfgdf", password="Admin@9009"):
...
def boot_to_page(page_name):
    ...
    if not locate_option(Key.RIGHT, [page_name], 30,2,readline=True):
...
def boot_os_centos_legacy(change_boot_mode=True):
    if change_boot_mode:
        ...
        assert boot_to_page(SutConfig.Msg.PAGE_BOOT_CN)
        assert select_option_value(Key.DOWN, [SutConfig.Msg.BOOT_MODE_CN], Key.DOWN, "Legacy", 6)
        send_keys(Key.SAVE_RESET)
        time.sleep(5)
    if not boot_with_hotkey(Key.F11, SutConfig.Msg.ENTER_BOOTMENU_CN, 200, SutConfig.Msg.HOTKEY_PROMPT_F11_CN):
        return
...
def boot_os_from_bm(os_type="linux"):
    if not boot_with_hotkey(Key.F7, SutConfig.Msg.ENTER_BOOTMENU_CN, 200, SutConfig.Msg.POST_MESSAGE):
        return
    if os_type.lower() == "linux":
        if not select_boot_option(Key.DOWN, SutConfig.Msg.LINUX_OS, 12,''):
            return
    elif os_type.lower() == "kylin":
        if not select_boot_option(Key.DOWN, SutConfig.Msg.KYLIN_OS, 12,''):
            return
    elif os_type.lower() == "centos":
        if not select_boot_option(Key.DOWN, SutConfig.Msg.CENTOS_OS, 12,''):
            return
    elif os_type.lower() == "uos":
        if not select_boot_option(Key.DOWN, SutConfig.Msg.UOS_OS, 12,''):
            return
    elif os_type.lower() == "windows":
        if not select_boot_option(Key.DOWN, SutConfig.Msg.WINDOWS_OS, 12, ''):
            return
    else:
...
def boot_to_shell():
    try:
        assert boot_to_setup()
        assert boot_to_page(SutConfig.Msg.PAGE_BOOT_CN)
        assert select_option_value(Key.DOWN, [SutConfig.Msg.INTERNAL_SHELL_CN], Key.DOWN, "打开", 6)
        time.sleep(2)
        send_keys(Key.SAVE_RESET)
        logging.info("Enable Shell seccessed.")
        assert boot_with_hotkey(Key.F11, SutConfig.Msg.ENTER_BOOTMENU_CN, 100, SutConfig.Msg.HOTKEY_PROMPT_F11_CN), "Press F11 failed."
...
def boot_to_dos():
    try:
        ...
        assert boot_with_hotkey(Key.F11, SutConfig.Msg.ENTER_BOOTMENU_CN, 200, SutConfig.Msg.HOTKEY_PROMPT_F11_CN)
...
def wait_message(msg, delay=150, pw_prompt=None, pw=None, cleanup=True,readline=True):
    ...
    while True:
        if Sut.BIOS_COM.session.in_waiting:
            try:
                ...
                # print('##############')
                if cleanup:
                    data = data + Sut.BIOS_COM.cleanup_data(data)
                    # print(data)
                    # print('++++++++++++++++++++++++++++====')
...
def back_to_setup_toppage(msg = 'Setup Configuration'):
    try:
        while True:
            logging.info('Navigating to top page,')
            send_key(Key.ESC)
            if wait_message(msg, delay=6):
...
def select_boot_option(key, optionname, try_counts, confirm_msg):
    ...
    while try_counts:
        time.sleep(1)
        if wait_message( patten, 2, cleanup=False,readline=True):
