...
from Hygon3000CRB.Base import Smbios
import re
...
def smbios():
    ...
    try:
        ...
        with open('Hygon3000CRB/Tools/SMBIOS/type0.txt', 'r') as f:
            content_log1 = f.read()
    ...
    for i in types:
        ...
        result = ReportGen.LogHeaderResult(tc)
        try:
            differs=Smbios.check_diff('{0}type{1}.txt'.format(SutConfig.Env.SMBIOS,str(i)),'Hygon3000CRB/Tools/SMBIOS/type{0}.txt'.format(str(i)))
            if len(differs)==0:
                result.log_pass()
            else:
