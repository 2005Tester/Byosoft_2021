...
def execute_command(ssh, command, decoding=''):
    if ssh.login():
        logging.info("Sending: {0}".format(command))
        stdin, stdout, stderr = ssh.ssh_client.exec_command(command, timeout=600)
        if decoding:
...
def execute_command_limit(ssh, command, delay=290, decoding=''):
    ...
    while True:
        cmd_result = execute_command(ssh, command, decoding)
        end_time = time.time()
