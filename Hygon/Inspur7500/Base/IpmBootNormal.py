...
def uefi_pxe_once_nor():
    ...
    assert SetUpLib.enter_menu(Key.DOWN,SutConfig.Msg.PAGE_BOOT_CN,10)
    time.sleep(1)
    SetUpLib.send_key(Key.RIGHT)
    time.sleep(1)
    SetUpLib.send_key(Key.LEFT)
    if SetUpLib.wait_message('<Legacy> *Boot Mod',3,readline=True):
    ...
    BmcLib.enable_serial_normal()
    if SetUpLib.wait_message(SutConfig.Msg.POST_MESSAGE):
        if not SetUpLib.wait_message(SutConfig.Ipm.BOTH_PXE_MSG, 20,readline=False):
    ...
    SetUpLib.send_key(Key.LEFT)
    if boot_mode=='Legacy':
        if SetUpLib.wait_message('<Legacy>.*Boot Mode',5,readline=True):
            stylelog.fail('Ipmitool修改UEFI PXE启动一次，第二次之后启动模式没有改变')
            return True
        else:
            stylelog.fail('Ipmitool修改UEFI PXE启动一次，第二次之后启动模式改变')
            return
    else:
        if SetUpLib.wait_message('<UEFI>.*Boot Mode',5,readline=True):
...
def uefi_hdd_once_nor():
    ...
    BmcLib.enable_serial_normal()
    if SetUpLib.wait_message(SutConfig.Msg.POST_MESSAGE):
        if SetUpLib.wait_message(SutConfig.Ipm.OS_MSG,2):
    ...
    BmcLib.enable_serial_normal()
    if SetUpLib.wait_message(SutConfig.Msg.POST_MESSAGE):
        if not SetUpLib.wait_message(SutConfig.Ipm.BOTH_USB_MSG, 20,readline=False):
...
def uefi_usb_once_nor():
    ...
    BmcLib.enable_serial_normal()
    if SetUpLib.wait_message(SutConfig.Msg.POST_MESSAGE):
        if SetUpLib.wait_message(SutConfig.Ipm.BOTH_PXE_MSG, 20,readline=False):
...
def legacy_pxe_once_nor():
    ...
    time.sleep(1)
    SetUpLib.send_key(Key.LEFT)
    if SetUpLib.wait_message('<Legacy> *Boot Mode',3,readline=True):
        logging.info('当前启动模式为Legacy')
        boot_mode = 'Legacy'
    else:
        logging.info('当前启动模式为UEFI')
...
def legacy_hdd_once_nor():
    ...
    BmcLib.enable_serial_normal()
    if SetUpLib.wait_message(SutConfig.Msg.POST_MESSAGE):
        if SetUpLib.wait_message(SutConfig.Ipm.OS_MSG,2,readline=False):
...
def legacy_pxe_always_nor():
    ...
    if re.search('<Legacy>.*Boot Mode', datas):
        ...
        return
    data = datas.replace('►', '').split('Boot Order')[2]
    options = []
    for i in data.split('  '):
        if i != '':
            options.append(i.strip(' '))
