...
def boot_to_setup(key=Key.DEL):
    ...
    while try_counts:
        ...
        if not SetUpLib.boot_with_hotkey_only(key, SutConfig.Msg.SETUP_MESSAGE, 200, SutConfig.Msg.POST_MESSAGE):
            reboot()
            try_counts -= 1
        else:
...
def boot_with_hotkey(key, msg, timeout, hotkey_prompt):
    ...
    while try_counts:
        enable_serial_normal()
        logging.info("Waiting for Hotkey message found...")
        if not SetUpLib.boot_with_hotkey_only(key, msg, timeout, hotkey_prompt):
            reboot()
            try_counts -= 1
        else:
...
def boot_to_boot_menu(key=Key.F11):
...
def boot_to_linux_os(key=Key.F11):
    if not boot_with_hotkey(key, SutConfig.Msg.ENTER_BOOTMENU, 200, SutConfig.Msg.POST_MESSAGE):
        return
...
def boot_to_internal_shell(key=Key.F11):
    boot_to_setup()
...
def boot_to_windows_os(key=Key.F11):
    if not boot_with_hotkey(key, SutConfig.Msg.ENTER_BOOTMENU, 200, SutConfig.Msg.POST_MESSAGE):
        return
...
def secure_boot_3404():
    ...
    try:
        ...
        SetUpLib.send_data_enter('123456789012-465789098765-32101234-6')
...
def secure_boot_3405():
    ...
    try:
        ...
        SetUpLib.send_data_enter('11111111-2222-3333-4444a1234567890ab')
        assert SetUpLib.wait_message(SutConfig.Sec.ERROR_CHARACTERS, 3), '输入错误格式的GUID没有提示格式错误'
        SetUpLib.send_key(Key.ENTER)
        time.sleep(2)
        SetUpLib.send_key(Key.ENTER)
