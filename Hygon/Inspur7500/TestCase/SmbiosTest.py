...
def smbios():
    ...
    count = 1
    for i in types:
        tc = ('{0}'.format(str(count + 102)), '[TC{0}]SMBIOS Type {1}'.format(str(count + 102), i),
              '检查 SMBIOS Type {0}'.format(i))
        result = ReportGen.LogHeaderResult(tc)
        try:
            differs = Smbios.check_diff('{0}type{1}.txt'.format(SutConfig.Env.SMBIOS, str(i)),
                                        'Inspur7500/Tools/SMBIOS/type{0}.txt'.format(str(i)))
            if len(differs) == 0:
                result.log_pass()
            else:
