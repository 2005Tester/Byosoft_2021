...
def boot_to_setup():
    ...
    while try_counts:
        ...
        if not boot_with_hotkey_only(Key.DEL, SutConfig.Msg.PAGE_MAIN_CN, 200, SutConfig.Msg.POST_MESSAGE):
...
def boot_os_from_bm(os_type="linux"):
    if not boot_with_hotkey(Key.F11, SutConfig.Msg.ENTER_BOOTMENU_CN, 200, SutConfig.Msg.POST_MESSAGE):
        return
...
def locate_menu(key, option_path, try_counts):
    ...
    logging.info("Go to setup menu:{0}".format(option_path))
    for option in option_path:
        if option in [SutConfig.Msg.PAGE_MAIN_CN,SutConfig.Msg.PAGE_ADVANCED_CN,SutConfig.Msg.PAGE_SECURITY_CN,SutConfig.Msg.PAGE_BOOT_CN,SutConfig.Msg.PAGE_EXIT_CN]:
            assert back_to_setup_toppage()
            assert boot_to_page(option)
        else:
            logging.info("Locate menu: {0}".format(option))
