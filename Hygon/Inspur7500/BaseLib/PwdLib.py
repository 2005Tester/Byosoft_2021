...
pw_incorrect = 'Incorrect Password|Incorrect password'#SetUp校验密码错误提示信息
pw_short = 'Please enter enough characters'#密码长度小于最少字符提示信息
pw_is_same = 'Password Rule Error'#管理员用户密码相同
pw_be_diff = 'The new password should be different'#
pw_is_history = 'Password has been used by the latest 5 times'#密码与前5次相同
pw_len_dif = 'Character length is not equal'#密码检查打开，长度不匹配
pw_user_only = 'Please set Administrator Password first'#未设置管理员密码直接设置用户密码
pw_simple = "Warning|Passwords need to contain upper and lower case letters"#密码字符类型不匹配
pw_not_same = "Passwords are not the same"#两次密码输入不一致
pw_user_del = 'User Rights Cannot Delete Password'#用户权限删除用户密码
pw_expire = 'Password expired'#密码过期
pw_lock = 'The password is incorrect and the screen is locked'#密码输错达到上限
pw_lock_60 = 'Please Reset or Wait 60 seconds|Please wait 60 seconds'#锁住60秒
pw_lock_180 = 'Please Reset or Wait 180 seconds|Please wait 180 seconds'#锁住180秒
pw_invalid = 'Invalid Password'#POST校验密码输错提示


class PW:
    DEFAULT_PW = 'YdNfv#X86$'
    MIN = 8
    MAX = 20
    ADMIN = None
    USER = None
...
def set_password(new_pw, result, old_pw=None, expect=None, confirm_pw=None):
    ...
    try:
        ...
        if expect == pw_user_only and not result:
            assert SetUpLib.wait_message(expect), f'{expect} not found'
            return True
        if old_pw:
        ...
        if expect in [pw_short, pw_len_dif]:
            if not SetUpLib.wait_message(expect, 2):
                SetUpLib.send_data_enter(confirm_pw)
                if not SetUpLib.wait_message(expect, 2):
                    logging.info(f'{expect} not found')
                    return
            SetUpLib.send_key(Key.ENTER)
            return True
        ...
        if result:
            ...
            expect = expect if expect else f"{pw_is_same}|{pw_is_history}|{pw_expire}|{pw_incorrect}|{pw_simple}|{pw_not_same}"
            assert SetUpLib.wait_message(expect), f'Msg not found: {expect}'
            SetUpLib.send_keys([Key.ENTER])
...
def del_psw(pw, result, expect=None):
    try:
        ...
        if result:
            ...
            if expect == pw_user_del:
                assert SetUpLib.wait_message(pw_enter_new, 5), '{} not found'.format(pw_enter_new)
                SetUpLib.send_keys([Key.ENTER, Key.ENTER])
                assert SetUpLib.wait_message(expect), f'{expect} not found'
                SetUpLib.send_keys([Key.ENTER])
            else:
                expect = expect if expect else f"{pw_incorrect}"
...
hdd_pw_enter_psw = 'Enter current HDD (?:Admin )*Password|Enter Hdd Password User Password|Enter (?:new|current) Hdd Password'  # 删除密码要求输入当前密码提示信息
...
def check_psw_post(pw=None, expect=None, timeout=10, key=None):
    ...
    try:
        if key:
            ...
            SetUpLib.send_data_enter(pw)
        expect = expect if expect != None else hdd_pw_incorrect
        assert SetUpLib.wait_message(expect, timeout), f'Msg not found:{expect}'
        if expect in [hdd_pw_incorrect, hdd_pw_limit_post, hdd_pw_escape, pw_lock, pw_expire]:
            SetUpLib.send_key(Key.ENTER)
        time.sleep(1)
        return True
    except Exception as e:
